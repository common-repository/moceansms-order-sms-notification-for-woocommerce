let caretPosition = 0;
jQuery(function ($) {
    $("#moceansms_setting_form").on("submit", function (event) {
        var sender_id = $("#moceansms_setting\\[moceansms_woocommerce_sms_from\\]").val().trim();
        if ($.isNumeric(sender_id) && sender_id.length > 15) {
            alert('Message From is too long, max 15 digits for numeric SMS sender.');
    ...
    $("#moceansms_admin_setting\\[moceansms_woocommerce_admin_sms_recipients\\]").focusout(function () {
        var admin_mobile_no = $("#moceansms_admin_setting\\[moceansms_woocommerce_admin_sms_recipients\\]").val().trim();
    ...
    const setupPhoneHelper = function () {
        let selectedValue = $("#moceansms_multivendor_setting\\[moceansms_multivendor_selected_plugin\\]").val();
        let phoneFieldLocation = '[edit profile page > MoceanAPI WooCommerce > phone]';
        ...
        $("#moceansms_multivendor_setting\\[multivendor_helper_desc\\]").html(helperText);
    };

    $("#moceansms_multivendor_setting\\[moceansms_multivendor_selected_plugin\\]").change(setupPhoneHelper);
    setupPhoneHelper();
    ...
    $('#mocean_sms\\[open-keywords\\]').click(function (e) {
        ...
        if ($('#moceansms_new_billing_field') && $('#moceansms_new_billing_field').val() !== '') {
            let newFields = $('#moceansms_new_billing_field').val().split(',');
    ...
    $('#mocean_sms\\[open-keywords-low-product-stock\\]').click(function (e) {
        const type = $(e.target).attr('data-attr-type');
        const target = $(e.target).attr('data-attr-target');

...
class MoceanSimpleMembership implements Moceansms_PluginInterface, Moceansms_Register_Interface {
    /*
    ...
    */
    ...
    public static function plugin_activated() {
        ...
        try {
            ...
            $log->add("MoceanSMS", "Failed to import classes/class.swpm-utils-member.php");
            $log->add("MoceanSMS", "Failed to import classes/class.swpm-utils-membership-level");
            return false;
        }
    ...
    public function get_plugin_settings($with_identifier = false) {
        $settings = array(
            ...
            "moceansms_automation_sms_template_recurring_payment_received" => moceansms_get_options("moceansms_automation_sms_template_recurring_payment_received", $this->get_option_id()),
            "moceansms_automation_sms_template_payment_cancelled"          => moceansms_get_options("moceansms_automation_sms_template_payment_cancelled", $this->get_option_id()),
        );
    ...
    private function get_sms_template_fields() {
        return array(
            array(
                'name'    => 'moceansms_automation_sms_template_recurring_payment_received',
                'label'   => __( 'Recurring payment received', MOCEANSMS_TEXT_DOMAIN ),
                'desc'    => sprintf('Customize your SMS with <button type="button" id="moceansms-open-keyword-%1$s-[dummy]" data-attr-type="pending" data-attr-target="%1$s[moceansms_automation_sms_template_recurring_payment_received]" class="button button-secondary">Keywords</button>', $this->get_option_id() ),
                'type'    => 'textarea',
                'rows'    => '8',
                'cols'    => '500',
                'css'     => 'min-width:350px;',
                'default' => __( 'Hi [first_name], your subscription of [total_amount] for [membership_level_name] via [payment_gateway] is [payment_status].', MOCEANSMS_TEXT_DOMAIN )
            ),
            array(
                'name'    => 'moceansms_automation_sms_template_payment_cancelled',
                'label'   => __( 'Membership cancellation or end of term', MOCEANSMS_TEXT_DOMAIN ),
                'desc'    => sprintf('Customize your SMS with <button type="button" id="moceansms-open-keyword-%1$s-[dummy]" data-attr-type="pending" data-attr-target="%1$s[moceansms_automation_sms_template_payment_cancelled]" class="button button-secondary">Keywords</button>', $this->get_option_id() ),
                'type'    => 'textarea',
                'rows'    => '8',
                'cols'    => '500',
                'css'     => 'min-width:350px;',
                'default' => __( 'Hi [first_name], your membership for [membership_level_name] has expired or cancelled. Renew now to retain access.', MOCEANSMS_TEXT_DOMAIN )
            ),
        );
    }
    ...
    private function schedule_reminders($member, $ipn_data, $status) {
        ...
        $this->log->add("MoceanSMS", "Member ID: {$member->member_id}");

        if( $member->account_state != 'active') {
            $this->log->add("MoceanSMS", "member status is not active. member status: {$member->account_state}");
        ...
        if( $membership_expiry_timestamp == PHP_INT_MAX) {
            // life time account
            $this->log->add("MoceanSMS", "This is a lifetime account. Aborting");
            return;
        }
        ...
        $this->log->add("MoceanSMS", "Membership expiry timestamp: {$membership_expiry_timestamp}");
    ...
    {
        $this->log->add("MoceanSMS", "User ID: {$member_id}");
    ...
    {
        $this->log->add("MoceanSMS", "Universal payment data received");
        $this->log->add("MoceanSMS", print_r($ipn_data, true));
    ...
    {
        $this->log->add("MoceanSMS", "Subscription payment data received");
        $this->log->add("MoceanSMS", print_r($ipn_data, true));
        ...
        // paypal
            ...
            $this->log->add("MoceanSMS", "member_id is empty, this is a new account, nothing to do here");
            return;
        }
        ...
        $this->log->add("MoceanSMS", "member_id: {$member_id}");
        ...
        if($enable_notifications === "on") {
            ...
            if(!empty($send_on) && is_array($send_on)) {
                if(array_key_exists($status, $send_on)) {
                    $this->log->add("MoceanSMS", "enable_notifications for {$status}: on");
                    $member = SwpmMemberUtils::get_user_by_id($member_id);
    ...
    {
        $this->log->add("MoceanSMS", "Cancelled or Expired Payment");
        $this->log->add("MoceanSMS", print_r($ipn_data, true));
        ...
        if( empty($ipn_data['member_id']) ) {
            $this->log->add("MoceanSMS", "member_id is empty, aborting...");
            return;
        }
    ...
    public function send_customer_notification($member, $ipn_data, $status)
    {
        ...
        $moceansms_seting = new Moceansms_WooCommerce_Setting();
        $countries = $moceansms_seting->get_countries();

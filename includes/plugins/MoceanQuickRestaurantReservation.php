...
class MoceanQuickRestaurantReservation implements Moceansms_PluginInterface, Moceansms_Register_Interface {
    /*
    ...
    */
    ...
    private function get_sms_template_fields() {
        return array(
            array(
                ...
                'cols'    => '500',
                'css'     => 'min-width:350px;',
                'default' => __( 'Greetings [qrr_user_name], your reservation for [qrr_party] on [qrr_date_formatted] is [qrr_booking_status]', MOCEANSMS_TEXT_DOMAIN )
            ),
            array(
            ...
            array(
                'name'    => 'moceansms_automation_sms_template_rejected',
                'label'   => __( 'Rejected SMS message', MOCEANSMS_TEXT_DOMAIN ),
                'desc'    => sprintf('Customize your SMS with <button type="button" id="moceansms-open-keyword-%1$s-[dummy]" data-attr-type="pending" data-attr-target="%1$s[moceansms_automation_sms_template_rejected]" class="button button-secondary">Keywords</button>', $this->get_option_id() ),
                'type'    => 'textarea',
                'rows'    => '8',
                'cols'    => '500',
                'css'     => 'min-width:350px;',
    ...
    private function get_reminder_fields() {
        return array(
            array(
                ...
                'label'   => __( 'Send reminder to customer before reservation', MOCEANSMS_TEXT_DOMAIN ),
    ...
    private function get_sms_reminder_template_fields() {
        return array(
            ...
            array(
                ...
                'cols'    => '500',
                'css'     => 'min-width:350px;',
                'default' => __( 'Greetings [qrr_user_name], your reservation for [qrr_party] on [qrr_date_formatted] is [qrr_booking_status]. - custom', MOCEANSMS_TEXT_DOMAIN )
            ),
        );
    }
    ...
    public function get_plugin_settings($with_identifier = false)
    {
        $settings = array(
            ...
            "moceansms_automation_sms_template_rejected"    => moceansms_get_options("moceansms_automation_sms_template_rejected", $this->get_option_id()),
        );
    ...
    public function send_sms_reminder($booking_id, $status)
    {
        ...
        $this->log->add("MoceanSMS", "Booking status: {$booking->get_status()}");
        $this->log->add("MoceanSMS", "Status: {$status}");

        if(strpos($booking->get_status(), 'confirmed') === false) {
            $this->log->add("MoceanSMS", "Booking status is not confirmed");
            return;
        }
        ...
        if($now_timestamp >= $booking_timestamp) {
            $this->log->add("MoceanSMS", "Booking date is in the past");
            return;
        }
    ...
    public function send_sms_on_updated_booking(int $post_id, WP_Post $post, bool $update)
    {
        ...
        update_option("test_mocean", $post);

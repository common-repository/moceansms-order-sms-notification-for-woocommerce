...
class Moceansms_WooCommerce_Notification {
	protected $log;
	...
	public function send_sms_woocommerce_order_status_changed( $order_id, $old_status, $new_status ) {
		$this->log->add( 'MoceanSMS', 'Order status changed": old status: ' . $old_status . ' , new status: ' . $new_status );
	}

	public function woocommerce_payment_complete( $order_id ) {
		$this->log->add( 'MoceanSMS', 'Payment completed' );
	}

	public function woocommerce_payment_complete_order_status( $order_id ) {
		$this->log->add( 'MoceanSMS', 'Completed order status' );
	}
	...
	public function send_customer_notification( $order_id, $status ) {
        if ( moceansms_get_options( 'moceansms_woocommerce_suborders_send_sms', 'moceansms_customer_setting', 'off' ) == 'off') {
            return;
        }
		if ( ! in_array( $status, moceansms_get_options( 'moceansms_woocommerce_send_sms', 'moceansms_customer_setting', array() ) ) ) {
			return;
		}
		...
		if (Moceansms_Multivendor_Factory::$activatedPlugin == "wc_marketplace")
		{
			...
			if( $is_suborder ) {
				...
				$send_sms_flag = false;
				if ( moceansms_get_options( 'moceansms_woocommerce_suborders_send_sms', 'moceansms_customer_setting', 'off' ) == 'on' ) {
					$send_sms_flag = true;
				}
			}
		}
		//Checking if multivendor is "dokan"
		if (Moceansms_Multivendor_Factory::$activatedPlugin == "dokan")
		{
			//checking if it's a suborder
			$dokan_suborders = dokan_is_sub_order($order_id);
			if($dokan_suborders)
			{
                $this->log->add("MoceanSMS", "order id({$order_id}) is dokan suborder: {$dokan_suborders}");
		...
		if (Moceansms_Multivendor_Factory::$activatedPlugin == "yith")
		{
            $this->log->add("MoceanSMS", "Plugin activated: Yith");
			//checking if it's a suborder
			$yith_suborders =  wp_get_post_parent_id($order_id) ;
			if($yith_suborders)
			{
                $this->log->add("MoceanSMS", "order id({$order_id}) is yith suborder: {$yith_suborders}");
				//Do not send sms when it's sub order
				$send_sms_flag = false;
				if ( moceansms_get_options( 'moceansms_woocommerce_suborders_send_sms', 'moceansms_customer_setting', 'off' ) == 'on' ) {
					$send_sms_flag = true;
		...
		if($send_sms_flag)
		{
			$message = moceansms_get_options( 'moceansms_woocommerce_sms_template_' . $status, 'moceansms_customer_setting', '' );
			if ( empty( $message ) ) {
				$message = moceansms_get_options( 'moceansms_woocommerce_sms_template_default', 'moceansms_customer_setting', '' );
			}
			if ( empty( $message ) ) {
				return;
			}

            $sms_recipient_setting = moceansms_get_options("moceansms_woocommerce_send_sms_to", "moceansms_customer_setting");
            ...
            if( isset($sms_recipient_setting['billing-recipient']) ) {
                ...
                if ( $customer_billing_phone !== false ) {
                    $this->log->add( 'MoceanSMS', 'Customer\'s billing phone number (' . $order_details->get_billing_phone() . ') in country (' . $order_details->get_billing_country() . ') converted to ' . $customer_billing_phone );
                } else {
                    $customer_billing_phone = $order_details->get_billing_phone();
                }
                MoceanSMS_SendSMS_Sms::send_sms( '', $customer_billing_phone, $message );
            }
            ...
            if ( isset($sms_recipient_setting['shipping-recipient']) ) {
                ...
                if ( $customer_shipping_phone !== false ) {
                    $this->log->add( 'MoceanSMS', 'Customer\'s shipping phone number (' . $order_details->get_shipping_phone() . ') in country (' . $order_details->get_shipping_country() . ') converted to ' . $customer_shipping_phone );
                } else {
                    $customer_shipping_phone = $order_details->get_shipping_phone();
                }
                MoceanSMS_SendSMS_Sms::send_sms( '', $customer_shipping_phone, $message );
            }
	...
	public function send_admin_notification( $order_id, $status ) {
        if ( moceansms_get_options( 'moceansms_woocommerce_admin_suborders_send_sms', 'moceansms_admin_setting', 'off' ) == 'off') {
            return;
        }
		//v1.1.18 add selection for sending admin notification on which status
		if ( ! in_array( $status, moceansms_get_options( 'moceansms_woocommerce_admin_send_sms_on', 'moceansms_admin_setting', array() ) ) ) {
			return;
		}
		...
		if (Moceansms_Multivendor_Factory::$activatedPlugin == "wc_marketplace")
		{
			...
			if( $is_suborder ) {
				$send_sms_flag = false;
				if ( moceansms_get_options( 'moceansms_woocommerce_admin_suborders_send_sms', 'moceansms_admin_setting', 'off' ) == 'on' ) {
					$send_sms_flag = true;
				}
			}
		}
		...
		if($send_sms_flag){
            $message = moceansms_get_options( 'moceansms_woocommerce_admin_sms_template', 'moceansms_admin_setting', '' );
            $message = $this->replace_order_keyword( $message, $order_details, 'admin', $status );
			$admin_phone = trim( moceansms_get_options( 'moceansms_woocommerce_admin_sms_recipients', 'moceansms_admin_setting', '' ) );
			//Get default country v1.1.17
			$admin_country = moceansms_get_options('moceansms_woocommerce_country_code', 'moceansms_setting', '' );
				...
				foreach ( $phone_no_array as $number ) {
					if ( $number != '' ) {
						...
						if ( $phone_with_country_code !== false ) {
							$this->log->add( 'MoceanSMS', 'Admin\'s phone number (' . $number . ') in country (' . $admin_country . ') converted to ' . $phone_with_country_code );
						} else {
						...
						MoceanSMS_SendSMS_Sms::send_sms('', $admin_phone_no, $message );
					}
				}
			}
		}
    ...
    public function send_admin_low_stock_notification($product, $status)
    {
        ...
        $this->log->add("MoceanSMS", "send admin notification on low stock enabled");

        $message = moceansms_get_options("moceansms_woocommerce_admin_sms_template_{$status}", 'moceansms_admin_setting');
	...
	protected function check_and_get_phone_number( $phone_number, $country ) {
		...
		if(empty($country)){
            // $this->log->add( 'MoceanSMS', 'COUNTRY EMPTY~~~~~~');
			$country = $selected_country_code;
		}
        $country = strtoupper($country);
        if(get_option("moceansms_domain_reachable")) {
            $check_phone_number_request_url = "https://dashboard.moceanapi.com/public/mobileChecking?mobile_number={$phone_number}&country_code={$country}";
        }
        else {
            $check_phone_number_request_url = "https://183.81.161.105:443/public/mobileChecking?mobile_number={$phone_number}&country_code={$country}";
        }


		$response = wp_remote_get($check_phone_number_request_url, array( 'sslverify' => false ));
        $this->log->add("MoceanSMS", "request url: {$check_phone_number_request_url}");
		if ( is_array( $response ) ) {
			...
			$this->log->add( 'MoceanSMS', "check number api err response: {$customer_phone_no}" );

			return false;
		}

		$this->log->add( 'MoceanSMS', 'check number api timeout, continue send without formatting' );

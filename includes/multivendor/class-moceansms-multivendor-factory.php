...
class Moceansms_Multivendor_Factory {
	/* @var Moceansms_WooCoommerce_Logger $logger */
	public $log;
	...
	private function __construct( Moceansms_WooCoommerce_Logger $log = null ) {
		if ( $log === null ) {
	...
	/**
	 * @return Abstract_Moceansms_Multivendor|bool
	 */
	...
	protected function getMultivendorManager() {
		$selected_setting = moceansms_get_options( 'moceansms_multivendor_selected_plugin', 'moceansms_multivendor_setting', 'auto' );
		...
		if ( $selected_setting === 'auto' ) {
            ...
            // $this->log->add("MoceanSMS", sprintf("auto detected multivendor plugin: %s", self::$activatedPlugin));
		} else {
